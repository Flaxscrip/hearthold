...
case "${1:-default}" in
  recall)
    ...
    # sandbox's ollama container. Fast on qwen2.5:3b (non-thinking instruct) — answers land in seconds.
  ...
  default | "")
    ...
    printf '\033[2m   live RAG recall over the partitions:  ./deploy/sandbox/run-kb.sh recall  (Ollama-backed)\033[0m\n'
    ;;
