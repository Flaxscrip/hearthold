...
#
# A compliance authority issues an AccreditedInvestor credential to the Sovereign; a verifier then challenges the
...
ISSUER_ROOT=/data/compliance-authority             # a DISTINCT data root → a distinct issuer identity
...
case "${1:-setup}" in
  setup)
    ...
    echo "▸ Compliance authority (issuer) — a distinct identity via its own data root ($ISSUER_ROOT)"
    ISSUER_DID=$(issuer init | tee /dev/stderr | did)
    echo "▸ Issue AccreditedInvestor → the Sovereign"
    OUT=$(issuer issue "$SOV_DID" AccreditedInvestor 'authority=Meridian Capital Compliance' tier=accredited); echo "$OUT"
...
✓ Prove flow ready.
  ...
  Compliance authority (issuer): $ISSUER_DID
  Schema (AccreditedInvestor): $SCHEMA_DID
  Credential:             $CRED_DID
...
Now, in TWO terminals:
  ...
  B)  ./deploy/sandbox/run-prove.sh verify          # the verifier requests → the Signet prompts in A
      (type 1379 in A to approve → B prints ✓ VERIFIED with tier=accredited; blank to deny)
EOF
    ;;
  signet)
    PIN="${2:-1379}"
  ...
  verify)
    ...
    . "$STATE"
    echo "Verifier → prove the Sovereign holds a AccreditedInvestor (tier=accredited)…"
    ver verify "$SOV_DID" "$SCHEMA_DID" "$ISSUER_DID" tier=accredited
    ;;
