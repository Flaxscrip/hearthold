...
|---|---|---|---|
| `HEARTHOLD_REGISTRY` | was `hyperswarm` → **now `local`** | **YES (fixed)** | **RESOLVED at the Hearthold layer** (`config.ts` + `.env.example`): `local` is DB-only, never gossiped, so private data can't propagate regardless of the node's topic. The keymaster's `defaultRegistry` = this, so it flows through every op (issuance included) — the sandbox already ran the full credential flow offline on `local`. Publishing is a deliberate opt-in. Archon's reference node stays public `hyperswarm`; **we do not change Archon** — Hearthold just doesn't inherit its default. **Framed as a topology choice** (single-node → `local`; separate nodes that must resolve each other → `hyperswarm` + unique topic) in [`DEPLOYMENT.md`](DEPLOYMENT.md#two-registry-topologies--pick-by-node-count-not-by-fear) — kept `local` (flaxscrip, 2026-07-28) after Aegis flagged its feature-gaps, which are vanilla-keymaster and already fixed via our `ephemeralRegistry` override. |
