...
|---|---|---|---|
| `HEARTHOLD_REGISTRY` | **`hyperswarm`** | **NO** | A `createId`/`createAsset` on the default **gossips to mainnet permanently** (Sevenfold already orphaned test identities this way). Candidate: default `local` — "born local, promoted never" — and make promotion to a gossip registry explicit. **Biggest L5 item.** |
