...
model. The public surface is an Emissary (Emissary); the brain behind it is an ordinary private Warden.*

## The idea, and the fork we didn't take

We want a **hosted Knowledge Base** an authorized community can query — and update —
...
The right design moves the public surface to **the role that is already meant to be public: the Emissary**
(Emissary). The Emissary is defined as the world-facing emissary that *sees in, projects out, carries
delegated authority, and holds no secret.* A public Emissary serving a query portal is textbook Emissary. The
Warden never faces the public.

> **This is the projector pattern, inverted.** Where *prove* is
> `verifier → Emissary relays → Sovereign/Signet approves → Emissary carries the proof out`, a *KB query* is
> `authorized Sovereign → Emissary relays → Warden authorizes + recalls → Emissary carries the answer out`.
> The Emissary is the world-facing carrier in both directions; the same seam does both.

## Architecture

```
 authorized Sovereign ──(web portal / DIDComm)──▶  public Emissary  ──DIDComm──▶  private Warden
   (a community member)                             (carries; no secret)       (authorizes; recalls;
   ◀──────── answer + citations ─────────────────────────────────────────      holds the KB; local AI)
```

- **The Warden stays private** — home/community-bound, holds the Knowledge Base, runs a **local-only** model
...
- **The public Emissary is a portal and a carrier** — it terminates the web/DIDComm connection, forwards the
  ...
  **community Emissary** acts as the community's public emissary.
...
Authorization lives in the *private* Warden, not the public Emissary — the Emissary only proves *who* is asking
and carries.
...
| **Public relay** | the **projector** pattern (`emissary/handler.ts`) |
...
hosted portal over the Emissary.
...
- **Invariant I — community brain ≠ personal vault.** The KB holds *shared* knowledge; it **never** holds a
  ...
  not become, a store of members' private histories. This is the line between a community *brain* and a
...
- **Invariant II — no query attribution retained.** The Warden reads a query **in memory only** to answer
  ...
  default.** Retaining per-DID query attribution would let the community host reconstruct a member's interest
...
**The honest part.** The community's host still runs the Warden and, at request time, sees the query *in
memory* to answer it (unavoidable — it must read the question). That is a coherent **librarian** posture:
the community's brain, reached through the community's portal — **not** a personal privacy-vault claim. What the
...
- **Grows to:** multi-Sovereign (add members), a community/public GUI, portable **VMC** membership (present a
  community-membership credential to be admitted to the group), and the prove→contribute bridge.

## Demo vehicle — the Drake Gamers Guild Knowledge Base

Community members reach the community's public Emissary, authenticate with their `did:cid`, and query the KB
(*"what's the raid schedule?"*, *"who's the current champion?"*); authorized officers update it;
authorization is community membership; and facts can be **proven** out of it. It's a tangible, multi-party,
reachable demonstration — and the natural driver for the community-manager GUI and the multi-Sovereign
future.
