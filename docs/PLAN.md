...
- **Warden** (home Keeper / Swordsman / Soulbis lineage) — always-on, home-bound custodian.
  ...
  model; mints proofs.
- **Emissary** (Companion / Mage / Soulbae lineage) — mobile envoy. Two jobs: **witnessing**
...
- **Emissary** — a separate `did:cid` identity, wallet at `~/.hearthold/emissary/` (so
  `backupId`/`recoverId` can later move it to phone/browser).
- **Delegation** — Warden issues Emissary a revocable `HearthholdDelegation` credential granting
  scoped query rights. Emissary presents it (with a fresh challenge/response) to get answers.
...
- **Transport**: direct HTTP over **Tailscale** — *not* dmail (dmail notices would leak the
  Emissary↔Warden relationship on the registry). Archon for trust; private socket for bytes.
...
flow, the **DTG credential set**, the **TRQP trust registry**, the **Emissary projector**, the **Signet**
...
1. **Identities** ✅ — Hearthold Keymaster wiring; `Warden` + `Emissary` DIDs (`init`).
2. **Delegation handshake** ✅ — `HearthholdDelegation` VC issue/accept + challenge/response;
   tested live (`e2e:delegation`). CLI: `warden delegate` / `emissary accept`.
3. **Transport + Emissary → store** ✅ — **DIDComm v2** transport (`Transport` seam); authcrypt
   ...
   (`e2e:submission` + two-process CLI). CLI: `warden serve` / `emissary submit`.
...
- **A2 — Sovereign co-sign** ✅: sensitive claims route through a **direct Warden↔Sovereign channel**
  (the Emissary is never in the authorization path, §7.7); the Sovereign **signs** the approval
...
The whole prove side is clickable in the Emissary app (*Prove a claim* → Signet approval → inspector).
...
- **Next:** structured extraction (extend the classifier seam to pull facts / entities / dates);
  a recall control endpoint + a GUI surface (Emissary/Warden "ask your vault"); better vector store
...
(an Emissary's autonomy ceiling). Interop with an independent TRQP deployment verified. See
[trust-graph-and-delegation.md](trust-graph-and-delegation.md).

### W — Emissary as projector  ✅ (per-device pending)
The **Emissary** is the world-facing projector (PVM Mage): a verifier contacts `emissary serve`, which
...
(`emissary/handler.ts`, `e2e:projector`, `e2e:inward-registry`). **Still to do:** **per-device Emissaries**
(one Sovereign, many Emissaries) with **kind-scope enforcement**. See the actors table in
...
### KB — Knowledge Base via a public Emissary portal + a private Warden  ◐ first increment built
A shared, authorized **Knowledge Base** that authorized Sovereign(s) can **query and update** — WITHOUT
touching the home Warden's invariant. The public surface is a **public Emissary** (Emissary): the community's
...
This is the projector pattern *inverted* — where prove is `verifier → Emissary relays → Sovereign approves →
Emissary carries out`, a KB query is `authorized Sovereign → Emissary relays → Warden authorizes + recalls →
Emissary carries out`. Multi-tenancy lives in the Emissary (holds no secret = safe to share), not the Warden.
...
provenance-stamped), the Emissary relay = the built **projector** pattern, and facts can be **proven** from
...
- **First increment** ✅ (`core/kb.ts`, `warden/kb.ts`, `emissary/kb-relay.ts`, `e2e:kb`): a KB Warden
  serves a shared KB; a member **authenticates** by signing the request over a Warden-issued nonce
  (`signKbRequest`/`verifyKbRequestSignature` — DID control proven end-to-end, so the relaying Emissary can't
  ...
  stamped). The public Emissary (`makeKbRelayHandler`) forwards and holds nothing. Verified live: member
...
- **Deployable CLI + daemon** ✅: `warden kb-init <kbId>` (creates read/write groups) · `kb-grant` /
  ...
  queries the sphere KB through the Emissary portal, three real processes.
- **Web portal** ✅ (`emissary/kb-portal-server.ts` + `emissary kb-web`; `apps/kb-portal`): the public
  ...
  and fetches to the Emissary. The browser produces the byte-for-byte same signed `KbRequestStatement` the
  ...
  Emissary.* **Still to do:** multi-KB per Warden.
- **Assurance / factor 2** ✅ (registry-governed step-up): the **Trust Registry** declares required
  ...
  over DIDComm — the Emissary is never on that channel, so it can't forge/replay), gated by a fresh
...
- **Grows to:** multi-Sovereign (add members to the group), a sphere/public GUI, and a **prove→contribute
  bridge** (publish a consented, derived fact from a personal vault into the shared KB).
- **Demo vehicle:** the Drake Gamers Guild Knowledge Base — members query/update via the sphere's public
  Emissary, authorization = sphere membership (VMC / group), facts provable. Drives the sphere-manager GUI.
...
review): **I — sphere brain ≠ personal vault** (the KB holds shared knowledge, never a member's 7th
...
Emissary fan-out via an Archon group. Optional predicate proofs for facts about data the Warden does
not issue.
