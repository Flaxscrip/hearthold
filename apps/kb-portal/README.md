# Knowledge Portal — the public Emissary's browser face
...
The signed request goes to the **Emissary** (`emissary kb-web`), which relays it over DIDComm to the private
**Warden**, which authenticates end-to-end, authorizes (a KB trust-registry group), and serves.

```
 browser (member's wallet, signs)  ──HTTP──▶  Emissary: emissary kb-web  ──DIDComm──▶  Warden (KB brain)
...
```bash
# 0. provision (once): a KB Warden, a Emissary, and grant a member DID
...
# 2. the public Emissary web bridge (HTTP → DIDComm)
HEARTHOLD_PASSPHRASE=… HEARTHOLD_WARDEN_DID=<wardenDid> emissary kb-web 4313
...
| Var | Default | Meaning |
|-----|---------|---------|
| `VITE_PORTAL_URL` | `http://127.0.0.1:4313` | the Emissary `kb-web` HTTP bridge |
...
The Emissary bridge binds to loopback by default; set `HEARTHOLD_PORTAL_HOST=0.0.0.0` (or a tailnet
address) on `emissary kb-web` to expose it.
...
**Login endpoints** (on the Emissary, `emissary kb-web`): `POST /api/kb/login/start` → challenge;
...
carries the session token. Set `HEARTHOLD_PORTAL_PUBLIC_URL` on `emissary kb-web` so the callback baked
...
```
 browser ──HTTP──▶  Emissary (emissary kb-web)  ──DIDComm──▶  Warden (warden serve)  ──HTTP──▶  Ollama
...
- **Warden** on **flaxlap.local** — holds the KB, classifies/embeds/recalls. Set its Ollama via
  ...
  ```bash
  export HEARTHOLD_DATA_ROOT=~/.hearthold-kb          # fresh root → a NEW KB Warden + Emissary
...
- **Emissary** (`emissary kb-web`) on **archon.social** — the public web face; relays to the Warden's DID over
  DIDComm. It touches neither Ollama nor any secret. Build the static app (`npm run build` → `dist/`),
  serve it behind the archon.social web server with `VITE_PORTAL_URL` → the Emissary bridge and
  `VITE_GATEKEEPER_URL` → the node's gatekeeper.
- Provision a fresh **KB Warden + Emissary + KB group** for the new database (new identities), then
