 ...
 *
 * The trust relationship between Warden and Witness is realized as an Archon challenge/response
 * keyed on the delegation credential:
 *
 *   1. Warden issues a HearthholdDelegation VC to the Witness (see credentials.ts).
 *   2. Witness accepts it into its wallet.
 *   3. Warden creates a CHALLENGE that requires a delegation VC issued by itself.
 *   4. Witness creates a RESPONSE, proving control of its DID and possession of the delegation.
 ...
 * conforming to the delegation schema. Returns the challenge DID for the Witness to answer.
...
/**
 * Witness answers a challenge, gathering matching credentials from its wallet and proving control
 * of its DID. Returns the response DID. The Witness must be the current identity on the handle.
 */
...
/** Warden verifies a Witness's response. A verified result clears the CHALLENGE tier. */
